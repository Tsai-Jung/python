...
while True:
    ...
    if results.pose_landmarks :
        ...
        if(righthand[1]<rightinner[1] and lefthand[1]<leftinner[1] and lockall==0 and nose[2]>-0.8):
            if(lockallcount>100 and nose[2]>-0.8):
                lockall=1
                time.sleep(3)
                lockallcount=0
            else:
            ...
            if num==1:
                ...
                if(lefthip3[1]<lefthip2[1]<lefthip1[1] and righthip3[1]<righthip2[1]<righthip1[1]
                   and rightknee3[1]<rightknee2[1]<rightknee1[1] and leftknee3[1]<leftknee2[1]<leftknee1[1]
                   and abs(lefthip[1]-lefthand[1])*0.9<abs(leftshoulder[1]-lefthand[1])
                    ...
                    time.sleep(0.2)
                    pyautogui.press('space')
                elif((lefthip[1]+leftknee[1])/2<lefthand[1] and (righthip[1]+rightknee[1])/2<righthand[1]):
                    print('换角色')
                    pyautogui.keyUp('w')
                    pyautogui.keyDown('s')
...
##                        C1234=1
            ...
            if num1==1:
                ...
                if(rightheel3[1]<rightheel2[1]<rightheel1[1]
                   and abs(lefthip[1]-lefthand[1])*0.8<abs(leftshoulder[1]-lefthand[1])
                   and abs(righthip[1]-righthand[1])*0.8<abs(rightshoulder[1]-righthand[1])):
                    print('走路')
                    pyautogui.keyDown('w')
                    lockwalk=0
...
##                    lockwalk=0
            ...
            if num2==1:
                if(righthand[0]<leftshoulder[0]):
                    print('左转')
                    mouse_move(-20-turnLeft, 0)
                elif(lefthand[0]>rightshoulder[0]):
                    print('右转')
                    mouse_move(20+turnRight, 0)
                else:
                    turnLeft=0
                    turnRight=0
    ...
    cv2.resizeWindow("Image", 1800, 1000)    # 设置长和宽
