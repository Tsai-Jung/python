...
while True:
    ...
    # 调整窗口大小
    cv2.namedWindow("WX:DesignProgram", 0)  # 0可调大小，注意：窗口名必须imshow里面的一窗口名一直
    cv2.resizeWindow("WX:DesignProgram", win32api.GetSystemMetrics(win32con.SM_CXSCREEN), win32api.GetSystemMetrics(win32con.SM_CYSCREEN))    # 设置长和宽
    cv2.imshow("WX:DesignProgram", img)
    #cv2.imshow("Canvas", imgCanvas)
    cv2.waitKey(1)
    
    # 点击窗口X按钮关闭窗口，窗口名字要对应
    if cv2.getWindowProperty('WX:DesignProgram',cv2.WND_PROP_VISIBLE) < 1:        
       break 
